...
public class Config {
    ...
    private static final ForgeConfigSpec.DoubleValue SKILLET_COOK_TEMPERATURE = BUILDER.comment("煎锅适配加热配方温度（默认200）").comment("如果不想让物品添加进煎锅加热，给物品添加#farmerstfc:cant_cook").defineInRange("skilletCookTemperature", 200.0, 200, 2000);

    private static final ForgeConfigSpec.DoubleValue STOVE_TEMPERATURE = BUILDER.comment("烤炉提供群峦热源可达到的最高温度（默认450）").defineInRange("stoveTemperature", 450.0, 200, 2000);
